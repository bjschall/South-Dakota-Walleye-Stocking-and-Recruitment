...
#model
m1new<- brm(bf(CPUE2~Stocked + (1|Waterbody2),
            ...
            #sample_prior = "only",
            file="Stocked_comparison1_hurdle.rds",
...
posts.new <- add_epred_draws(m1new, re_formula = NA,
                             newdata = m1new$data %>% distinct(Stocked) %>% 
...
ggplot()+
  ...
  scale_x_continuous(labels=c(round(-2*sd(wae3$Hectares)+mean(wae3$Hectares),1), 
                              round(mean(wae3$Hectares),1), 
                              round(1*sd(wae3$Hectares)+mean(wae3$Hectares),1), 
                              round(2*sd(wae3$Hectares)+mean(wae3$Hectares),1),                              
                              round(3*sd(wae3$Hectares)+mean(wae3$Hectares),1)), 
                     breaks=c(-2,0,1,2,3))+
  theme(axis.text = element_text(size=12), 
        axis.title = element_text(size=14), 
...
(p1<-ggplot()+
  ...
  geom_ribbon(data=posts_env_GDD,aes(x=GDD_std, y=.epred,ymin=.lower, ymax=.upper), alpha=0.4)+
  theme_classic()+
  xlab("Growing degree days") +
  ylab("Age-2 Walleye CPGN")+
  scale_x_continuous(labels=c(round(-2*sd(wae3$GDD)+mean(wae3$GDD), 1), 
                              round(-1*sd(wae3$GDD)+mean(wae3$GDD), 1), 
                              round(mean(wae3$GDD), 1), 
                              round(1*sd(wae3$GDD)+mean(wae3$GDD), 1), 
                              round(2*sd(wae3$GDD)+mean(wae3$GDD), 1),
                              round(3*sd(wae3$GDD)+mean(wae3$GDD), 1)), 
...
(p2<-ggplot()+
    ...
    geom_ribbon(data=posts_env_WSI,aes(x=WSI_std, y=.epred,ymin=.lower, ymax=.upper), alpha=0.4)+
    theme_classic()+
    xlab("Winter severity index") +
    ylab(NULL)+
    scale_x_continuous(labels=c(round(-2*sd(wae3$WSI)+mean(wae3$WSI),1), 
                                round(-1*sd(wae3$WSI)+mean(wae3$WSI),1), 
                                round(mean(wae3$WSI),1), 
                                round(1*sd(wae3$WSI)+mean(wae3$WSI),1), 
                                round(2*sd(wae3$WSI)+mean(wae3$WSI),1)), 
                       breaks=c(-2,-1,0,1,2))+
    theme(axis.text = element_text(size=12), 
          axis.title = element_text(size=14))+
    ylim(0,40))
...
(p3<-ggplot()+
    ...
    geom_ribbon(data=posts_env_SP,aes(x=SpringPrecip_std, y=.epred,ymin=.lower, ymax=.upper), alpha=0.4)+
    theme_classic()+
    xlab("Spring precipitation (mm)") +
    ylab(NULL)+
    scale_x_continuous(labels=c(round(-1*sd(wae3$SpringPrecip)+mean(wae3$SpringPrecip),1), 
                                round(mean(wae3$SpringPrecip),1), 
                                round(1*sd(wae3$SpringPrecip)+mean(wae3$SpringPrecip),1), 
                                round(2*sd(wae3$SpringPrecip)+mean(wae3$SpringPrecip),1),
                                round(3*sd(wae3$SpringPrecip)+mean(wae3$SpringPrecip),1),
                                round(4*sd(wae3$SpringPrecip)+mean(wae3$SpringPrecip),1)), 
